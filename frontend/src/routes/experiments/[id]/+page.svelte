...
{/if}
	...
	<!-- D12 Inc 5a: queued-capacity detail — the busy/idle ratio. The download
	     progress (5c) now lives INSIDE the ActivityHeart above (it surfaces for a
	     running experiment mid-pull too, not only while queued). -->
	{#if activity?.run_phase === 'queued' && activity.capable_worker_count != null}
	...
	<nav class="tabs" aria-label="Experiment sections">
		...
		<button class="tab" class:active={tab === 'benchmark'} onclick={() => { tab = 'benchmark'; loadBenchmarks(); }}>Benchmark</button>
	</nav>
